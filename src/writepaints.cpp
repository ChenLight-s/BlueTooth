...
//    this->resize(deskRect.width()/2,deskRect.height()/2);
...
//时间事件函数
void WritePaints::timerEvent(QTimerEvent *event)
{
//    qDebug()<<event;
    (void)event;
    killTimer(event->timerId());
    qDebug()<<event<<"定时完成";
    emit paintCircularButton->clicked();
...
//        QPushButton *btn0 = new QPushButton("返回",this);
//        btn0->adjustSize();
...
//        paintCircularButton->setText("123");
        ui->toolBar->addSeparator();
        //工具栏添加控件
        QPushButton *btn1 = new QPushButton("画矩形",this);
        btn1->adjustSize();
        ui->toolBar->addWidget(btn1);
        ui->toolBar->addSeparator();
        connect(btn1,&QPushButton::clicked,ui->toolBar,[=](){on_paintRectangleButton_clicked();});
}
...
//    mainwindow->sendMessageClicked(data);
//    serial.write(data.toUtf8());
//    client->sendMessage(data.toUtf8());
    qDebug()<<data.toUtf8();
//  设置运行激光功率
...
//    mainwindow->on_Laser(paintYuan->ui->minLaser_Control->text().toUtf8());
    qDebug()<<"圆";
}
...
//    serial.write(data.toUtf8());
    qDebug()<<data.toUtf8();
//  设置运行激光功率
    mainwindow->on_Laser(paintYuan->ui->maxLaser_Control->text().toUtf8());
//    左边
    data += "G01G91X0Y";
...
void WritePaints::on_pushButton_clicked()
{
